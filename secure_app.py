...
if not str_app.session_state.logged_in:
    ...
    # ------------------ تبويب تسجيل الدخول ------------------
    with auth_action[0]:
        account_type = str_app.radio("سجل دخولك كـ:", ["شركة رئيسية (Company)", "عميل تابع لشركة (Client)"],
                                     key="login_role")
        str_app.markdown("---")
                    ...
                    with conn.cursor() as cursor:
                        if account_type == "شركة رئيسية (Company)":
                            ...
                            if result:
                                ...
                                str_app.session_state.user_data = {"name": result["name"], "email": result["email"],
        ...
        if reg_type == "شركة جديدة":
            ...
            if signup_btn and reg_name and reg_email and reg_password and reg_comp_id:
                try:
                    ...
                    conn.commit()
                    str_app.success(
                        "🎉 تم تسجيل شركتك بنجاح في قاعدة البيانات! يمكنك الآن الانتقال لتبويب تسجيل الدخول.")
            ...
            reg_client_comp_id = str_app.text_input("أدخل الرقم التعريفي للشركة التي تتبع لها:",
                                                    key="reg_client_comp_id")
...
# --- 4. عرض لوحة التحكم بعد تسجيل الدخول الناجح ---
    ...
    if str_app.session_state.user_role == "company":
        ...
        tab1, tab2, tab3 = str_app.tabs(
            ["📊 المراقبة الحية وسجل العمليات", "👥 إدارة حسابات العملاء الفعليين", "📈 تحليل البيانات والأمن التكيفي"])
        ...
        with tab1:
            ...
            if str_app.session_state.logs:
                ...
                decrypted_output = system.decrypt_data(last_log["cipher_text"], last_log["current_key"],
                                                       last_log["current_iv"], last_log["current_tag"])
    ...
    # -----------------------------------------------
        ...
        input_type = str_app.radio("اختر نوع البيانات المراد حمايتها وإرسالها:",
                                   ["نص مكتوب فقط", "رفع ملف (مع إمكانية إضافة نص مرافق)"])
